...
<form class="eleads-form eleads-form--seo" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
    <?php wp_nonce_field('eleads_save_seo_settings', 'eleads_seo_nonce'); ?>
    <input type="hidden" name="action" value="eleads_save_seo_settings">

    <section class="eleads-section" aria-labelledby="eleads-seo-title">
        <h2 id="eleads-seo-title"><?php echo esc_html__('SEO сторінки', 'e-leads-for-woocommerce'); ?></h2>

        <?php if (! $is_available) : ?>
            <div class="notice notice-warning inline">
                <p>
                    <?php echo esc_html__('SEO сторінки недоступні для поточного API ключа E-Leads.', 'e-leads-for-woocommerce'); ?>
                </p>
            </div>
        <?php endif; ?>
        ...
        <div class="eleads-seo-settings eleads-card">
            <div class="eleads-seo-settings__header">
                <label class="eleads-toggle">
                    ...
                    <span class="eleads-toggle__label"><?php echo esc_html__('Увімкнути SEO сторінки E-Leads', 'e-leads-for-woocommerce'); ?></span>
                </label>
            </div>

            <div class="eleads-copy-row">
                <label class="eleads-field eleads-field--wide">
                    <span><?php echo esc_html__('URL sitemap', 'e-leads-for-woocommerce'); ?></span>
                ...
                <div class="eleads-copy-row__actions">
                    ...
                    <a class="button button-secondary" href="<?php echo esc_url($seo_sitemap_url); ?>" target="_blank" rel="noopener noreferrer">
                        <?php echo esc_html__('Відкрити', 'e-leads-for-woocommerce'); ?>
                    </a>
                </div>
            </div>

            <p class="description">
                <?php echo esc_html__('Після увімкнення модуль створить sitemap і відкриє сторінки /e-search/{slug} для SEO контенту з E-Leads.', 'e-leads-for-woocommerce'); ?>
            </p>
        </div>
    </section>
    ...
    <p class="submit eleads-submit eleads-submit--seo">
        <button type="submit" class="button button-primary" <?php disabled(! $is_available); ?>>
            <?php echo esc_html__('Застосувати', 'e-leads-for-woocommerce'); ?>
        </button>
    </p>
</form>
